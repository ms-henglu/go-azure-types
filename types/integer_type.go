...
func (t *IntegerType) Validate(body interface{}, path string) []error {
	...
	if t.MinValue != nil && v < *t.MinValue {
		return []error{ErrorCommon(path, fmt.Sprintf("value is less than %d", *t.MinValue))}
	}
	if t.MaxValue != nil && v > *t.MaxValue {
		return []error{ErrorCommon(path, fmt.Sprintf("value is greater than %d", *t.MaxValue))}
	}
	return nil
}

func (t *IntegerType) FilterReadOnlyFields(i interface{}) interface{} {
	return i
}
