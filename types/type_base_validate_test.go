...
var loader *AzureSchemaLoader

func init() {
	loader = DefaultAzureSchemaLoader()
}
			...
			Body: `
{
	...
	for index, data := range testData {
		...
		def, err := loader.GetResourceDefinition(data.ResourceType, data.ApiVersion)
		if err != nil {
			t.Fatal(err)
		}
		...
		if def != nil {
			...
			fmt.Printf("Running test for case %d, resource type: %s, api-version: %s\n", index, data.ResourceType, data.ApiVersion)
			fmt.Println(errors)
			if (len(errors) > 0) != data.Error {
				t.Errorf("expect error: %t, got error: %t for resource type: %s, api-version: %s", data.Error, len(errors) > 0, data.ResourceType, data.ApiVersion)
			}
		} else {
			t.Fatalf("failed to load resource definition for resource type: %s, api-version: %s", data.ResourceType, data.ApiVersion)
		}
	}
}
...
`,
	...
	for _, data := range testData {
		...
		if def != nil {
			res := (*def).FilterConfigurableFields(input)
...
`,
	...
	for _, data := range testData {
		...
		if def != nil {
			res := (*def).FilterReadOnlyFields(input)
			if !reflect.DeepEqual(res, output) {
				resJson, _ := json.Marshal(res)
				t.Errorf("expect %s got %s", data.Output, string(resJson))
			}
